    ...
    if (isEventShort) {
        ...
        return (
            <div className="relative flex h-full items-center justify-between gap-1 overflow-hidden px-1">
                ...
                {showBand && <div className="absolute top-0 left-0 h-full w-1 flex-shrink-0 rounded-l" style={{ backgroundColor: bandColor }} />}
                ...
                <div className={`flex-1 truncate text-xs leading-4 font-medium ${showBand ? "ml-2" : ""} ${isCompleted ? "opacity-60" : ""}`}>
                    {arg.event.title}
                </div>
    ...
    return (
        <div className="relative h-full overflow-hidden p-1">
            ...
            <div className={`h-full ${showBand ? "ml-2" : ""}`}>
                ...
                {durationMinutes < 45 ? (
                    <>
                        <div className={`truncate text-xs leading-5 font-semibold ${isCompleted ? "opacity-60" : ""}`}>{arg.event.title}</div>
                    ...
                    <>
                        <div className={`line-clamp-2 text-xs leading-5 font-semibold ${isCompleted ? "opacity-60" : ""}`}>{arg.event.title}</div>
