...
export const ObjectiveNode: React.FC<ObjectiveNodeProps> = ({ data, selected }) => {
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      {/* Objective card */}
      <div
        className={`relative flex h-[${nodeHeight}px] w-[${nodeWidth}px] cursor-pointer flex-col items-center justify-center rounded-xl bg-white p-3 shadow-lg transition-shadow hover:shadow-xl ${
          selected ? "border-2 border-gray-900 shadow-xl" : "border-2 border-gray-200"
        } ${isAchieved ? "border-green-500 bg-green-50" : ""}`}
        style={{
          width: nodeWidth,
          height: nodeHeight,
          borderColor: selected ? "#111827" : isAchieved ? "#10b981" : "#e5e7eb",
        }}
      >
        ...
        {/* Achievement badge */}
        {isAchieved && (
          <div className="absolute -right-2 -top-2 flex size-6 items-center justify-center rounded-full bg-green-500 text-xs font-bold text-white shadow-md">
            ✓
          </div>
        )}
      </div>

      {/* Connection handles - hidden but required for edges */}
