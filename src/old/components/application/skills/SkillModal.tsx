...
type SkillModalProps = {
  ...
  calculatedScores?: Array<{ criterion_id: string; score: number }>;
};
...
const StageRubricSection: React.FC<{
  ...
  isLocked: boolean;
  calculatedScores?: Array<{ criterion_id: string; score: number }>;
}> = ({ skill, stage, isCurrentStage, isLocked, calculatedScores }) => {
  ...
  const totalPossibleScore = rubric.criteria.reduce((sum, criterion) => sum + (criterion.weight ?? 100), 0);
  const currentScore = rubric.criteria.reduce((sum, criterion) => {
  ...
  return (
    <div className={`rounded-lg border-2 ${getStageBorderColor(stage)} ${getStageBgColor(stage)} ${isLocked ? "opacity-60" : ""}`}>
      {/* Stage header */}
      <button onClick={() => setIsExpanded(!isExpanded)} className="flex w-full items-center justify-between p-4 text-left transition-colors hover:bg-black/5">
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {rubric.criteria.map((criterion) => {
                    // Use calculated score if available, otherwise fall back to rubric score
                    const calculatedScore = calculatedScores?.find((s) => s.criterion_id === criterion.criterion_id);
                    const score = calculatedScore?.score ?? criterion.score ?? 0;
 ...
 */
export const SkillModal: React.FC<SkillModalProps> = ({ open, skill, onClose, onTicketClick, calculatedScores }) => {
  if (!open || !skill) return null;

  const currentStageIndex = STAGES_ORDER.indexOf(skill.current_stage);

  // Calculate overall progress as the average of all stage progress percentages
  // Each stage's progress is calculated independently based on its rubric criteria
  const stageProgressValues = STAGES_ORDER.map((stage) => {
    const rubric = skill.rubrics.find((r) => r.stage === stage);
    if (!rubric) return 0;

    const stageTotalPossible = rubric.criteria.reduce((sum, criterion) => sum + (criterion.weight ?? 100), 0);
    const stageCurrent = rubric.criteria.reduce((sum, criterion) => {
      const calculatedScore = calculatedScores?.find((s) => s.criterion_id === criterion.criterion_id);
      const score = calculatedScore?.score ?? criterion.score ?? 0;
      return sum + score;
    }, 0);

    return stageTotalPossible > 0 ? (stageCurrent / stageTotalPossible) * 100 : 0;
  });

  const overallProgress = stageProgressValues.reduce((sum, progress) => sum + progress, 0) / STAGES_ORDER.length;
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="space-y-4">
            {STAGES_ORDER.map((stage, index) => {
              ...
              // Calculate if previous stage is complete for locking logic
              let isPreviousStageComplete = true;
              if (index > 0) {
                const prevStage = STAGES_ORDER[index - 1];
                const prevRubric = skill.rubrics.find((r) => r.stage === prevStage);
                if (prevRubric) {
                  const prevTotalPossible = prevRubric.criteria.reduce((sum, criterion) => sum + (criterion.weight ?? 100), 0);
                  const prevCurrent = prevRubric.criteria.reduce((sum, criterion) => {
                    const calculatedScore = calculatedScores?.find((s) => s.criterion_id === criterion.criterion_id);
                    const score = calculatedScore?.score ?? criterion.score ?? 0;
                    return sum + score;
                  }, 0);
                  const prevProgress = prevTotalPossible > 0 ? (prevCurrent / prevTotalPossible) * 100 : 0;
                  isPreviousStageComplete = prevProgress >= 100;
                }
              }

              const isLocked = index > currentStageIndex && !isPreviousStageComplete;
