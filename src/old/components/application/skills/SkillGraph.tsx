...
export const SkillGraph: React.FC<SkillGraphProps> = ({
  ...
  useEffect(() => {
    setNodes((nds) =>
      nds.map((node) => {
        const isSelected = node.id === selectedSkillId || node.id === selectedObjectiveId || node.id === selectedStageId || node.id === selectedTicketId;
                ...
                : node.type === "ticket" && node.id === selectedTicketId
                  ? (() => {
                      ...
                      return {
                        ...node.data,
                        onUnlink:
                          connectedStage && onUnlinkTicket ? () => onUnlinkTicket(node.id, connectedStage.skill_id, connectedStage.stage_node_id) : undefined,
                      };
                    })()
                  : node.data,
        };
  ...
  const onNodeClick = useCallback(
    ...
    [onSkillClick, onObjectiveClick, onStageClick, onTicketClick, lastClickedNodeId, selectedSkillId, selectedObjectiveId, selectedStageId, selectedTicketId],
  );
