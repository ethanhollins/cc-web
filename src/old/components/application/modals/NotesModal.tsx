    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" aria-modal="true" role="dialog">
      ...
      >
        ...
        <div className="relative flex flex-1 flex-col overflow-hidden">
          ...
          <button
            onClick={onBack}
            className="absolute left-4 top-4 z-10 flex cursor-pointer items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600"
          ...
          {note && (
            <button
              ...
              className="absolute right-4 top-4 z-10 flex cursor-pointer items-center justify-center rounded-md p-2 text-gray-400 hover:bg-red-50 hover:text-red-600 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          {/* Note content */}
          <div className="flex-1 overflow-auto p-16 pb-20 pt-16">
            {noteLoading ? (
                ...
                {isEditMode ? (
                  ...
                  <div className="w-full">
                    <textarea
                      ...
                      className="max-h-[550px] min-h-[400px] w-full resize-none rounded-lg border border-gray-300 p-4 font-mono text-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500"
                  ...
                  <div className="w-full">
                    {/* Created timestamp */}
                    {note?.created_at && (
                      <div className="mb-4 border-b border-gray-100 pb-2 text-xs text-gray-500">Created: {formatTimestamp(note.created_at)}</div>
                    )}
                    ...
                    >
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          li: ({ children, className, ...props }) => {
                            ...
                            return (
                              <li className={`${className || ""} ${hasTaskList ? "task-list-item" : "standard-list-item"}`} {...props}>
                                {children}
                              </li>
                            );
                          },
                      ...
                      <div className="mt-4 text-xs text-gray-400 opacity-0 transition-opacity group-hover:opacity-100">Click anywhere to edit this content</div>
                    </div>
                  </div>
                )}
              </div>
                ...
                {isEditMode ? (
                  ...
                  <div className="w-full max-w-4xl">
                    ...
                    <textarea
                      ...
                      onChange={(e) => setEditContent(e.target.value)}
                      onBlur={handleTextareaBlur}
                      className="min-h-[400px] w-full resize-none rounded-lg border border-gray-300 p-4 font-mono text-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500"
                      placeholder="Enter your markdown content here..."
                      onInput={(e) => {
                        const target = e.target as HTMLTextAreaElement;
                        target.style.height = "auto";
          ...
          {messageQueue.length > 0 && (
            <div className="bottom-25 absolute left-6 right-6 rounded-lg border border-gray-200 bg-white p-3 shadow-lg">
              ...
              <div className="max-h-32 space-y-2 overflow-y-auto">
                {messageQueue.map((message) => (
                  <div key={message.id} className="flex items-center gap-2 rounded-full bg-blue-100 px-3 py-1.5">
                    <span className="flex-1 text-sm text-blue-900">{message.text.length > 60 ? `${message.text.slice(0, 60)}...` : message.text}</span>
                    {isProcessing && <div className="h-3 w-3 animate-spin rounded-full border-2 border-blue-600 border-t-transparent"></div>}
                  </div>
                ))}
              </div>
            </div>
