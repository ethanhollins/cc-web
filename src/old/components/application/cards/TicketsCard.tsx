...
export default function TicketsCard({ items, projects, selectedProjectKey, onProjectChange, onItemClick }: Props) {
  ...
  const projectTickets =
    items[projectKey]?.filter((ticket) => ticket.ticket_type.toLowerCase() !== "epic" && !["done", "removed"].includes(ticket.ticket_status.toLowerCase())) ||
    [];
  ...
  return (
    <Card className="flex-10 mb-2 min-w-[400px]">
      <CardHeader title="Project Tickets" className="text-violet-950" headerIcon={<Feather className="size-5 text-violet-800" />}>
        <div className="relative">
          <select
            value={projectKey}
            onChange={handleChange}
            className="max-w-[200px] appearance-none truncate whitespace-nowrap rounded-lg border border-gray-300 bg-white py-2 pl-3 pr-9 text-sm font-medium text-gray-900 shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-300"
          >
            {projects
            ...
            <li key={t.ticket_id} onClick={() => onItemClick?.(t)} className="cursor-pointer hover:bg-gray-50/50">
              <div className="flex items-center justify-between gap-2 px-4 py-3">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <p className="whitespace-nowrap text-xs text-gray-900">{t.ticket_key}</p>
                  <p className="overflow-hidden truncate whitespace-nowrap text-[15px] font-medium text-gray-900">{t.title}</p>
                </div>

                <div className="ml-4 flex items-center gap-3">
                  <span className={["whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-semibold", statusPillClasses(t.ticket_status)].join(" ")}>
                    {t.ticket_status}
                  </span>
                </div>
              </div>
