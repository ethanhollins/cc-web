...
export default function NotificationsCard({ items, newCount = items.filter((i) => i.unread).length }: Props) {
  ...
  return (
    <Card className="flex-6 mb-2 mt-2">
      <CardHeader title="Recent Notifications" className="text-blue-950" headerIcon={<Bell01 className="size-5 text-blue-800" />}>
        {!!newCount && <span className="select-none rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-900">{newCount} new</span>}
      </CardHeader>
      <ul>
        {items.map((n, idx) => {
          ...
          const content = (
            <>
              <div className="flex items-center gap-3">
                <IconFor type={n.type} />
                <p className="truncate whitespace-nowrap text-xs font-medium text-gray-900">{n.title}</p>
              </div>
            ...
            <li key={n.id}>
              <RowTag
                ...
                className={[
                  ...
                  n.href ? "outline-none ring-0 hover:bg-gray-50/50 focus:bg-gray-50/50" : "",
                ].join(" ")}
              >
                {content}
              </RowTag>
